...
/**
 * dashboard_admin.php
 * Updated: Integrated "Tentang" directly into the dynamic layout.
 */
include 'koneksi.php';
...
<body class="bg-[#fffaf5] text-stone-800 min-h-screen flex flex-row overflow-x-hidden">
    ...
    <aside class="w-72 bg-[#4a2c1d] text-white flex flex-col justify-between p-6 shrink-0 shadow-xl min-h-screen sticky top-0">
        <div class="space-y-8">
            ...
            <nav class="space-y-2" id="sidebar-nav">
                ...
                <!-- INTEGRATED TENTANG VIEW LINK -->
                <button onclick="switchView('tentang', this)" id="btn-tentang" class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs text-amber-200/70 hover:text-white text-left transition border-l-4 border-transparent">
                    ℹ️ Tentang
                </button>
            </nav>
        </div>
    ...
    <main class="flex-1 p-8 lg:p-12 max-w-7xl overflow-y-auto">
        ...
        <div id="view-kelola-content" class="dashboard-view hidden space-y-6">
            <div class="flex flex-wrap items-center gap-2 border-b border-stone-200 pb-3" id="tab-filter-bar">
                ...
                <button onclick="filterTableCategory('PARKIR', this)" class="px-4 py-2 bg-white border border-stone-200 text-stone-600 hover:bg-stone-50 text-xs font-semibold rounded-xl transition">🚗 Parkir</button>
            </div>
                            ...
                            <tr class="hover:bg-orange-50/30 transition-colors report-data-row" data-category="<?= htmlspecialchars($rowCatClean); ?>">
                                <td class="p-4 align-top">
                                    <?php if(!empty($row['foto'])): ?>
                                        <div class="relative w-16 h-16 cursor-pointer" onclick="openPhotoModal(this)" data-photo="<?= htmlspecialchars($row['foto']); ?>">
                                            <img src="<?= (strpos($row['foto'], 'data:image') === 0 || strpos($row['foto'], 'http') === 0) ? $row['foto'] : '../uploads/'.$row['foto']; ?>" class="w-16 h-16 object-cover rounded-xl border border-stone-200 shadow-sm">
                                        </div>
                                    <?php else: ?>
                                ...
                                <td class="p-4 align-top text-center">
                                    <span class="px-2 py-1 bg-stone-100 text-stone-600 border rounded-full text-[9px] font-black uppercase"><?= htmlspecialchars($row['status'] ?? 'Proses'); ?></span>
                                </td>
                                <td class="p-4 align-top">
                                    <div class="flex flex-col gap-1.5">
                                        <form action="update_status.php" method="POST"><input type="hidden" name="id_laporan" value="<?= $row['id_laporan']; ?>"><input type="hidden" name="status" value="Diterima"><button type="submit" class="w-full bg-green-600 text-white py-1 rounded-lg text-[9px] font-bold uppercase">Terima</button></form>
                                        <form action="update_status.php" method="POST"><input type="hidden" name="id_laporan" value="<?= $row['id_laporan']; ?>"><input type="hidden" name="status" value="Tidak Diterima"><button type="submit" class="w-full bg-amber-600 text-white py-1 rounded-lg text-[9px] font-bold uppercase">Tolak</button></form>
                                    </div>
                                </td>
                            </tr>
                            <?php endwhile; ?>
        ...
        <!-- ================= NEW VIEW 3: INTEGRATED TENTANG CONTENT ================= -->
        <div id="view-tentang-content" class="dashboard-view hidden">
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-stone-200/60 max-w-3xl space-y-6">
                <div>
                    <h3 class="text-lg font-black text-stone-900">Tentang Aplikasi Pengaduan Wisata</h3>
                    <p class="text-xs text-stone-500 mt-1">Sistem Informasi Layanan Aspirasi dan Keluhan Infrastruktur Pariwisata Kabupaten Madiun.</p>
                </div>
                <hr class="border-stone-100">
                <p class="text-xs text-stone-600 leading-relaxed">
                    Aplikasi ini dirancang sebagai platform internal back-office yang mempermudah admin memantau, memvalidasi, serta merespons berbagai keluhan fasilitas umum, pelayanan, kebersihan, hingga tata tertib parkir di destinasi wisata secara real-time.
                </p>
                <div class="bg-[#fffaf5] border border-orange-100 p-4 rounded-xl text-xs space-y-1">
                    <div class="flex justify-between"><strong>Versi Sistem:</strong> <span class="text-stone-500">v2.4.1 (Stable)</span></div>
                    <div class="flex justify-between"><strong>Engine Framework:</strong> <span class="text-stone-500">PHP 8.2 & TailwindCSS SPA Engine</span></div>
                    <div class="flex justify-between"><strong>Status Database:</strong> <span class="text-green-600 font-bold">Terhubung Aktif</span></div>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        function switchView(viewName, buttonElement) {
            ...
            document.getElementById('view-tentang-content').classList.add('hidden');
            
            // Clear layout style on other items
            ...
            // Set layout style on selected element
            ...
            if (viewName === 'ringkasan') {
                ...
                document.getElementById('view-tentang-content').classList.remove('hidden');
                document.getElementById('view-title').textContent = "Informasi Tentang Sistem";
                document.getElementById('view-subtitle').textContent = "Detail manajemen lisensi aplikasi pengaduan pariwisata madiun.";
            }
        }
